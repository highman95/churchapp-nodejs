...
module.exports = {
      ...
      (err0, lastMeeting, code = 400) => {
        ...
        db.beginTransaction((err1) => {
          ...
          db.query(
            ...
            this._createResultHandler(cb, statistic, meeting)
          );
        });
      }
    );
  ...
  findByStationAndMeetingTypeAndDate(station_id, meeting_type_id, held_on, cb) {
    if (typeof cb !== "function") {
      throw new Error("Callback is not defined");
    }
    ...
    db.query(
      ...
      this._findResultWithTaskHandler(cb)
    );
  },
  ...
  _createResultHandler(cb, statistic, meeting) {
    return (err, result) => {
      if (err) {
        return db.rollback(() =>
